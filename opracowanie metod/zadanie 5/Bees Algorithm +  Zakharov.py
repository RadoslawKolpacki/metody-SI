...
def zakharov(args):
    n = len(args)
    term1 = np.sum(args**2)
    term2 = np.sum(0.5 * (np.arange(n) + 1) * args)**2
    term3 = np.sum(0.5 * (np.arange(n) + 1) * args)**4
    return term1 + term2 + term3

class Bee:
    def __init__(self, bounds):
        self.position = np.random.uniform(bounds[0], bounds[1], size=len(bounds))
        self.score = zakharov(self.position)
...
bees_algo = BeesAlgorithm(zakharov, bounds, num_employed_bees, num_onlooker_bees, max_iterations)
