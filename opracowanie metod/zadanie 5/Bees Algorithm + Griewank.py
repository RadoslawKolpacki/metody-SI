...
def griewank(args):
    part1 = np.sum(args**2 / 4000)
    part2 = np.prod(np.cos(args / np.sqrt(np.arange(len(args)) + 1)))
    return 1 + part1 - part2

class Bee:
    def __init__(self, bounds):
        self.position = np.random.uniform(bounds[0], bounds[1], size=len(bounds))
        self.score = griewank(self.position)
...
# Example usage:
bounds = [-600, 600]  # Bounds for each variable
num_employed_bees = 50
num_onlooker_bees = 50
max_iterations = 100

bees_algo = BeesAlgorithm(griewank, bounds, num_employed_bees, num_onlooker_bees, max_iterations)
