...
def holder_table(args):
    x = args[0]
    y = args[1]
    term1 = -np.abs(np.sin(x) * np.cos(y) * np.exp(np.abs(1 - (np.sqrt(x**2 + y**2) / np.pi))))
    term2 = -np.abs(np.exp(np.abs(1 - (np.sqrt(x**2 + y**2) / np.pi))) * np.abs(np.sin(y) * np.cos(x)))
    return term1 + term2
...
bfoa = BacterialForagingOptimization(holder_table, bounds, num_bacteria, num_iterations, chemotactic_step_size, swim_length, tumble_rate)
